...
def train_model(mode="baseline", steps=500, n_candidates=8, accum_steps=4, is_dry_run=False):
    ...
    for step in range(steps):
        ...
        if mode in ["practical", "theoretical"]:
            active_rm.eval() 
...
def generate_all_answers(max_prompts=None):
    ...
    data = {
        ...
        "answers_theoretical": _generate("./llama_fpo_theoretical_weights")
    }
...
def evaluate_and_classify():
    ...
    print("Evaluating Baseline vs Theoretical...")
    win_b_t = evaluate_pair(client, prompts, data["answers_baseline"], data["answers_theoretical"], "Baseline", "Theoretical")
    
    print("Evaluating Theoretical vs Practical...")
    win_t_p = evaluate_pair(client, prompts, data["answers_theoretical"], data["answers_practical"], "Theoretical", "Practical")

    df = pd.DataFrame({
        'Prompt': prompts,
        'Ans_Baseline': data['answers_baseline'],
        'Ans_Practical': data['answers_practical'],
        'Ans_Theoretical': data['answers_theoretical'],
        'Category': categories,
        'Win_Base_v_Prac': win_b_p,
        'Win_Base_v_Theo': win_b_t,
        'Win_Theo_v_Prac': win_t_p
    })
    
    df['Len_Base'] = df['Ans_Baseline'].apply(lambda x: len(x.split()))
    df['Len_Prac'] = df['Ans_Practical'].apply(lambda x: len(x.split()))
    df['Len_Theo'] = df['Ans_Theoretical'].apply(lambda x: len(x.split()))
    ...
    print(pd.crosstab(df['Category'], df['Win_Base_v_Theo'], margins=True, margins_name="Total").to_string())
    
    print("\nTHEORETICAL VS PRACTICAL:")
    print(pd.crosstab(df['Category'], df['Win_Theo_v_Prac'], margins=True, margins_name="Total").to_string())

    print("\nVerbosity check:")
    print(f"Baseline: {df['Len_Base'].mean():.1f} words")
    print(f"Practical: {df['Len_Prac'].mean():.1f} words")
    print(f"Theoretical: {df['Len_Theo'].mean():.1f} words")
...
if __name__ == "__main__":
    ...
    if args.mode in ["train", "all"]:
        ...
        train_model(mode="theoretical", steps=steps_to_run, n_candidates=n_cands, is_dry_run=args.dry_run)
